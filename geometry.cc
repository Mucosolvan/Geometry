...
bool Position::operator==(const Position &pos2) const {
    return (x_coord == pos2.x_coord && y_coord == pos2.y_coord);
}
...
bool Vector::operator==(const Vector &vec2) const {
    return (x_coord == vec2.x_coord && y_coord == vec2.y_coord);
}
...
bool Rectangle::operator==(const Rectangle &rect2) const {
    return (w == rect2.w && h == rect2.h && position == rect2.position);
}
...
rect_pair Rectangle::split_horizontally(long place) const {
    ...
    Rectangle second = Rectangle(w, h - place, position + Vector(0, place));
    return {first, second};
}
...
rect_pair Rectangle::split_vertically(long place) const {
    ...
    Rectangle second = Rectangle(w - place, h, position + Vector(place, 0));
    return {first, second};
}
